...
@section('contents')
    <div class="d-flex align-items-center justify-content-between">
        <h2 class="mb-0">Surat Keterangan Usaha</h2>
        <a href="{{ route('usaha.create') }}" class="btn btn-primary">Buat Surat</a>
    </div>
    ...
    <table class="table table-hover mt-3">
        ...
        <tbody>
            ...
            @endphp
                ...
                @if($rs->id_surat == 2) {{-- Sesuaikan dengan ID jenis surat yang diinginkan --}}
                    <tr>
                        <td>{{ $counter++ }}</td>
                        <td>{{ $user->nama }}</td>
                        <td>{{ $user->nik }}</td>
                        ...
                        <td>
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <a href="{{ route('usaha.show', $rs->id) }}" class="btn btn-primary">Detail</a>
                                <a href="{{ route('usaha.edit', $rs->id) }}" class="btn btn-warning">Edit</a>
                                <form action="{{ route('usaha.destroy', $rs->id) }}" method="POST" onsubmit="return confirm('Delete?')" class="mr-1">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        @if ($rs->status_pengajuan != 4)
                                            <form action="{{ route('usaha.updateStatus', $rs->id) }}" method="post">
                                                @csrf
                                                @method('PUT')
                                        ...
                                        @else
                                            ...
                                            <a href="{{ route('admin.usaha.cetak', $rs->id) }}" target="_blank" class="btn btn-warning">Cetak</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
